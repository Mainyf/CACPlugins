...
object StorageMI : AbstractStorageManager() {
...
//                        list.pagination(pageIndex, pageSize)
    ...
    private fun getIslandCreature(uuid: UUID, creatureType: CreatureType): PlayerIsLandCreature? {
        return PlayerIsLandCreature.find {
            (PlayerIsLandCreatures.island eq uuid) and (PlayerIsLandCreatures.creatureType eq creatureType)
        }.firstOrNull()
    }

    fun getCreatureCount(uuid: UUID, creatureType: CreatureType): Int {
        return transaction {
            getIslandCreature(uuid, creatureType)?.count ?: 0
        }
    }

    fun setCreatureCount(uuid: UUID, creatureType: CreatureType, count: Int) {
        transaction {
            val islandCreature = getIslandCreature(uuid, creatureType)
            if (islandCreature == null) {
                val pIsLand = getPlayerIsland(uuid) ?: return@transaction
                PlayerIsLandCreature.newByID {
                    this.island = pIsLand.id
                    this.creatureType = creatureType
                    this.count = max(0, count)
                }
            } else {
                islandCreature.count = max(0, count)
            }
        }
    }

    fun addCreatureCount(uuid: UUID, creatureType: CreatureType, count: Int) {
        transaction {
            val islandCreature = getIslandCreature(uuid, creatureType)
            setCreatureCount(uuid, creatureType, (islandCreature?.count ?: 0) + count)
        }
    }

    fun removeCreatureCount(uuid: UUID, creatureType: CreatureType, count: Int) {
        transaction {
            val islandCreature = getIslandCreature(uuid, creatureType)
            setCreatureCount(uuid, creatureType, (islandCreature?.count ?: 0) - count)
        }
