...
fun <T : Any> SizedIterable<T>.pagination(pageIndex: Int, pageSize: Int): SizedIterable<T> {
    return pagination(pageIndex.toLong(), pageSize)
}

fun <T : Any> SizedIterable<T>.pagination(pageIndex: Long, pageSize: Int): SizedIterable<T> {
    return limit(pageSize, (pageIndex - 1L) * pageSize)
}